...
imageInput.addEventListener("change", () => {
  ...
  reader.onload = e => {
    ...
    ratingResult.textContent = `${hierarchyLabels[baseRating]} (PSL ${baseRating + 1})`;
  };
...
applyBtn.addEventListener("click", () => {
  ...
  ratingResult.textContent = `${hierarchyLabels[newRating]} (PSL ${newRating + 1})`;
});

simulateBtn.addEventListener("click", () => {
  const target = parseInt(hierarchySelect.value, 10);
  ratingResult.textContent = `${hierarchyLabels[target]} (PSL ${target + 1})`;
});
